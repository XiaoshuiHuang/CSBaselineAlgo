function []=RunJPMPCT(path,dataset)
...
% 
kinectPly=sprintf('%s/%s_p2.ply',path,dataset);

%scale
sfmPlyOri   =sprintf('%s/%s_p1.ply',path,dataset);
cmd=sprintf('ScaleBox.exe %s %s',kinectPly,sfmPlyOri);
system(cmd);

sfmPly   =sprintf('%s/%s_p1.plyscalse.ply',path,dataset);
...
Trmse=norm(T_compute-TG,'fro');
srmse=norm(s-sG,'fro');
Rmse=Trmse+srmse;
toc;
path1=sprintf('%s/%s_JRMPC_Fnorm_Result.txt',path,dataset);
dlmwrite(path1,Rmse,' ');
