function []=RunJRMPCS(path,dataset)
...
path1=sprintf('%s/%s/JRMPC_T_compute11.txt',path,dataset);
TT=[T;[1]];
T_compute=[RR;[0,0,0]];
T_compute=[T_compute,TT];
dlmwrite(path1,T_compute,' ');

%from point1 to point2, that is sfmPly to kinectPly. make sure the
%projection is the same.
cmd=sprintf('TransformPointCloudByT.exe %s %s %s',sfmPly,kinectPly,path1);
system(cmd);
toc;
close all
clear all
