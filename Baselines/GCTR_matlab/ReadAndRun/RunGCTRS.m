function [] = RunGCTRS(path,dataset)
...
% system(commandline);
% commandline=sprintf('SuperPixel.exe %s %s/%s/sfm',sfmPly,path,dataset);
% system(commandline);

kinectPly=sprintf('%s/%s/kinect.ply',path,dataset);
sfmPly=sprintf('%s/%s/sfm.ply',path,dataset);
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
saveCorrespondecePath=sprintf('%s/%s/correspondence3.txt',path,dataset);
if reverse
    X2=X2';
end
  [tmp match] = max(X2);
...
path1=sprintf('%s/%s/GCTR_Scale_compute.txt',path,dataset);
TT=[T;[1]];
T_compute=[R;[0,0,0]];
T_compute=[T_compute,TT];
T_compute=[T_compute;[s,0,0,0]];
dlmwrite(path1,T_compute,' ');
